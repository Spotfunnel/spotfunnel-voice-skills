...
- **Contact** — existing public phone number(s), email(s), any other contact channel the business publicises.
  ...
  **Email dual-use heuristic:** if a published email address has a generic local part (`cs@`, `info@`, `hello@`, `contact@`, `enquiries@`, `support@`, `team@`), it almost always serves multiple purposes (sales AND support, or all departments) regardless of how the page labels it. When you see one of these addresses listed under a specific category like "SALES", note it as serving both: e.g. `cs@example.com.au — labelled SALES on site but generic-named, likely also handles support [from site only]`. If the address is name-specific (`leo@`, `sarah.smith@`), trust the page's labelling.
- **Policies & pricing** — explicit statements of policy, prices, guarantees, terms. Examples: "$145 initial consult", "no-obligation quotes", "30-day satisfaction guarantee", "we don't bulk-bill", "minimum callout fee $90". Only capture what the source explicitly states — don't infer prices.
  ...
  **Strong rule on prices:** if the website publishes specific dollar amounts (plans, tiers, fees, hourly rates, package prices), **capture all of them** with the matching plan/product/service name. Don't punt to "see the pricing page" or "the team can quote" when the source itself lists numbers. If `/plans`, `/pricing`, or similar URLs were scraped, mine them carefully — the agent uses these on calls and won't have access to the page at runtime. Capture the smallest defensible unit that's published (e.g. "Unlimited Plan: $20 first month, $40/month thereafter, no setup fee, no lock-in [from site only]"). If the customer markets pricing as a feature on their site, the brain-doc must reflect that; otherwise the agent fumbles the most-asked question on calls.
