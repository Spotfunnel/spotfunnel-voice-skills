...
Telnyx provides the phone numbers (DIDs) and TeXML application that bridges PSTN calls to Ultravox.
...
2. **Generate an API key.** Telnyx Portal → **API Keys → Create API Key**. Save the key. Paste it into `.env`:
   ```
   TELNYX_API_KEY=KEY...
   ```
3. **Buy 3–5 DIDs (phone numbers)** in your country/region. **Phone Numbers → Search & Buy Numbers**.
...
4. **Create a TeXML application.** Telnyx Portal → **Voice → TeXML Applications → Create TeXML Application**.
   - Name it something memorable like `voice-ai-pool` or `<your-org>-pool`.
   - Set **Voice URL** to *either*:
     - your dashboard-server's TeXML webhook path (if it has one — depends on your dashboard-server build), OR
     - `https://app.ultravox.ai/api/agents/{agent_id}/telephony_xml` as a placeholder. The skill **rewires this per-customer at claim time**, pointing the claimed DID at that customer's freshly-created Ultravox agent. The initial value is overwritten on first claim.
   - Set **HTTP Method** to `POST`.
   - Set **Outbound Voice Profile** if you have one (optional for inbound-only).
   - Save. **Copy the TeXML application ID** from the URL or the app details (a numeric ID like `2750000000000123456`).
5. **Bind your purchased DIDs to this TeXML app.** For each DID: **Phone Numbers → My Numbers → click the DID → Voice tab → Connection: TeXML Application: `<your-app-name>`**. Save.
6. Paste the TeXML app ID into `.env`:
   ```
   TELNYX_POOL_TEXML_APP_ID=2750000000000123456
   ```

You'll know it worked when:
```bash
curl -sS -H "Authorization: Bearer $TELNYX_API_KEY" "https://api.telnyx.com/v2/texml_applications/$TELNYX_POOL_TEXML_APP_ID" | python3 -c "import json, sys; d = json.load(sys.stdin)['data']; print('app:', d['friendly_name']); print('voice_url:', d['voice_url'])"
```
returns the friendly name and voice URL you just configured.
...
```
returns either `[]` (empty table — fine) or a row count, with no auth errors.
